...
- [x] 版本号定稿 3.0.0：版本常量 / UA 字符串 / 产物名全部对齐
      （照 2.0 定稿动作：文档、产物名一次改齐，不留 preview/RC 残留）
- [x] CHANGELOG v3.0.0：新能力逐条全录 + **Bug 修复段**
      （P1–P3 过程中的修复随手回填，不许发版前补写）
- [x] 发行说明文件 `tools/release-body-3.0.0.md`（Markdown，CI 接文件上传）
- [x] README：路线图 3.0 行标 ✅、当前版本标记移到 3.0；工程质量段数字
      实测更新（包数/测试文件数/模板池规模）；能力矩阵加 3.0 能力行；
      主视觉换预置的 `assets/banner-3.0-exploit.png`
- [x] docs/产品说明.md 同步 3.0 能力；docs/开发文档.md 14 节改「已落地」实况
- [x] 数据资产：情报库如刷新则同步 Release 私有资产；full 版模板库打包清单
      加 network/dns/ssl 目录（文件数实测写死进 CI，防体积失控无感）

## P5 质量门禁（全绿才准进 P6）

- [x] `go test ./...` 全绿（新增 netx/exploit 相关包各带单元测试，与 52 文件基线合并计数）
- [x] CI `-race` 竞态门禁绿
- [x] govulncheck 零发现（P1 若引入新依赖，全量重扫）
- [x] fuzz 轮次照 2.0 口径：模板转换漏斗 + dsl 求值器 + **新增 netx 报文解析面**
- [x] P2 利用级正反例断言 + 未授权零利用请求断言（单测固化）；靶场农场双断言待 docker 靶场环境跑 regression_nightly（本机无靶场资产，列为发版前 CI/夜间任务）
- [x] Mimosa 深度审计收口（commit 前完整扫描结论，发现清零或逐项闭环）
- [x] 真机 e2e：授权目标全流水线跑通一遍，含「协议模板」段与 replay

## P6 发布流程（照 2.0 实际走法，坑位已标）

- [x] 提交纪律：`git add` 逐文件点名（**禁 `git add -A`，会卷临时文件**）；
      测试验证与 commit 分开跑（**管道吞退出码，红着提交坑**）
- [x] 提交身份核对：QQ 邮箱 + 姓名统一
- [x] 恢复 tag 自动构建（日常处于暂停态，2.0 同款操作）
