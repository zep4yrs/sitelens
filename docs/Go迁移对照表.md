...
| Nuclei 模板子集 | tools/import_nuclei.py + scanner/checks.py | internal/nuclei | YAML 直接装载（word/status/regex/头匹配/dsl 安全子集 internal/dsl），三路调度：tag 置顶 + 词面相关度排序（向量语义的轻量近似）+ 轮转覆盖 |
...
| PostgreSQL 知识库/检索（trgm） | 设计性替代：Go 用内存索引 + 文件存储；`sitelens migrate-pg` 可无损导入旧 PG 历史（幂等，实测 47 条） | 不回迁；trgm 检索以相关度排序近似 |
