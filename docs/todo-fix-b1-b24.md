...
## B25 服务指纹 RE2 不兼容模式全量兼容（新增，2026-09-12 分析）

实测 735 条被拒模式的构造分布：负向先行 `(?!...)` 671（主体）、占有量词/原子组 51、
正向先行 13、反向引用 1。机械转写不可行（负向先行是非正则构造），方案：

- [ ] **受限回溯匹配器** `internal/rex`（约 300 行）：支持 nmap 指纹子集
  （字面量/类/dot/锚点/交替/组/量词含惰性/负向先行/反向引用），单次尝试
  10 万步预算封顶防灾难性回溯；fpMatcher 编译时 RE2 失败者自动落到此引擎，
  其余照走 RE2 快路径
- [ ] 覆盖测试：735 条被拒模式逐条用回溯引擎编译并匹配冒烟；步数超限样例
  单测固化
- [ ] 验收：735 条全部有效（零丢弃），文档口径同步（B21 修正作废）
